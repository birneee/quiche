...
/// Asserts that a boolean expression is true at runtime, only if
/// debug_assertions are enabled.
///
/// Otherwise, the compiler is told to assume that the expression is always true
/// and can perform additional optimizations.
///
/// # Safety
///
/// The caller _must_ ensure this condition is never possible, otherwise the
/// compiler may optimize based on false assumptions and behave incorrectly.
#[macro_export]
macro_rules! assume {
    (false) => {
...
/// Traces a branch value if the `branch-tracing` feature is enabled. Otherwise
/// it is a no-op.
#[macro_export]
macro_rules! branch {
    ($cond:expr) => {{

...
pub trait Reader: Storage {
    ...
    /// Returns `true` when the reader represents logical bytes only and the
    /// caller can preserve flow without materializing payload.
    #[inline]
    ...
    /// Skips the data in the reader until `offset` is reached, or the reader
    ...
    fn with_max_data(&mut self, max_data: VarInt) -> Limit<'_, Self> {
        ...
        let max_buffered_len =
            max_buffered_len.as_u64().min(self.buffered_len() as u64) as usize;
        self.with_read_limit(max_buffered_len)
    }

    /// Limits the maximum amount of data that the caller can read from the
    /// reader
    #[inline]
    ...
    /// # Note
    ///
    /// `debug_assertions` must be enabled for these checks to be performed.
    /// Otherwise, the reader methods will simply be forwarded to `Self`.
    #[inline]

    ...
    R: Reader + ?Sized,
{
    ...
    fn read_chunk(
        ...
        // copy the returned chunk into another buffer so we can read the `inner`
        // state
        self.chunk.clear();
    ...
    R: Reader + ?Sized,
{
    ...
    fn is_virtually_buffered(&self) -> bool {
        self.inner.is_virtually_buffered()
    }
...
impl Snapshot {
    ...
    #[inline]
    fn check<R: Reader + ?Sized>(
        &self, reader: &R, dest_written_len: usize, chunk_len: usize,
    ) {
        assert!(

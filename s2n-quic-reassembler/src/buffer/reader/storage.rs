...
pub trait Storage {
    ...
    ///
    /// Implementations should either fill the `dest` completely or exhaust the
    /// buffered data.
    ///
    /// The storage also returns a `Chunk`, which can be used by the caller to
    /// defer copying the trailing chunk until later. The returned chunk
    /// must fit into the target destination. The caller must eventually
    /// copy the chunk into the destination, otherwise this data will be
    /// discarded.

...
pub trait Storage {
    ...
    ///
    /// If `false` is returned, the storage wasn't capable of this operation and
    /// a regular `put_*` call should be used instead.
    #[inline(always)]
    ...
    /// Returns the additional number of bytes that can be written to the
    /// storage
    ...
    ///
    /// Callers should check `SPECIALIZES_BYTES` before deciding to use this
    /// method. Otherwise, it might be cheaper to copy a slice into the
    /// storage and then increment the offset.
    #[inline]
    ...
    ///
    /// Callers should check `SPECIALIZES_BYTES_MUT` before deciding to use this
    /// method. Otherwise, it might be cheaper to copy a slice into the
    /// storage and then increment the offset.
    #[inline]
    ...
    /// Only allows a single write into the storage. After that, no more writes
    /// are allowed.
    ///
    /// This can be used for very low latency scenarios where processing the
    /// single read is more important than filling the entire storage with

...
use request::Request;
use request::RequestLen;
use slot::Slot;

/// The default buffer size for slots that the [`Reassembler`] uses.
///
/// This value was picked as it is typically used for the default memory page
/// size.
const MIN_BUFFER_ALLOCATION_SIZE: usize = 4096;
pub(super) const ZERO_CHUNK: [u8; 4096] = [0; 4096];

/// The value used for when the final size is unknown.
///
/// By using `u64::MAX` we don't have to special case any of the logic. Also
/// note that the actual max size of any stream is a `VarInt::MAX` so this isn't
/// a valid value.
...
impl Reassembler {
    ...
    /// Returns true if the buffer has completely been written to and the final
    /// size is known
    #[inline]
    ...
    /// Returns true if the buffer has completely been read and the final size
    /// is known
    #[inline]
    ...
    /// Records data at a certain offset without allocating payload.
    /// This is useful for benchmarking where actual payload is not needed.
    /// The buffer maintains the same reader-driven reassembly flow as
    /// [`write_at`].
    #[inline]
    pub fn write_at_len(
        &mut self, offset: VarInt, len: usize,
    ) -> Result<(), Error> {
        let mut request = RequestLen::new(offset, len, false)?;
        self.write_reader(&mut request)?;
        Ok(())
    }
    ...
    #[inline]
    pub fn write_at_fin(
        &mut self, offset: VarInt, data: &[u8],
    ) -> Result<(), Error> {
        ...
        R: Reader + ?Sized,
    {
        ...
        if let Err(err) = self.write_reader_impl(reader) {
            use core::any::TypeId;
            if TypeId::of::<R::Error>() !=
                TypeId::of::<core::convert::Infallible>()
            {
        ...
        R: Reader + ?Sized,
    {
        ...
        match (reader.final_offset(), self.final_size()) {
            ...
            (Some(final_offset), None) => {
                ...
                // make sure that we didn't see any previous chunks greater than
                // the final size
                ensure!(
            ...
            (None, Some(expected)) => {
                // make sure the reader doesn't exceed a previously known final
                // offset
        ...
        self.cursors.max_recv_offset =
            self.cursors.max_recv_offset.max(buffered_offset);
    ...
    where
        R: Reader + ?Sized,
    {
        // if the reader is empty at this point, just make sure it doesn't return
        // an error
        ...
        for idx in (0..self.slots.len()).rev() {
            let Some(slot) = self.slots.get(idx) else {
                unsafe {
                    // SAFETY: `idx` should always be in bounds, since it's
                    // generated by the range `0..slots.len()`
                    assume!(false);
                }
            };
    ...
    #[inline(always)]
    fn write_reader_at<R>(
        &mut self, reader: &mut R, mut idx: usize,
    ) -> Result<(), R::Error>
    where
        R: Reader + ?Sized,
    {
        ...
        while !reader.buffer_is_empty() {
            let Some(slot) = self.slots.get_mut(idx) else {
                unsafe {
                    // SAFETY: `idx` should always be in bounds, since it's
                    // provided by a range that was bound to
                    // `slots.len()`
                    assume!(false);
                }
            };
    ...
    fn write_reader_with_alloc<R>(
        &mut self, reader: &mut R, idx: &mut usize, filled_slot: &mut bool,
    ) -> Result<(), R::Error>
    where
        R: Reader + ?Sized,
    {
    ...
    fn unsplit_range(&mut self, range: core::ops::Range<usize>) {
        ...
        for idx in range.rev() {
            ...
            // if this slot was completed, we should try and unsplit with the next
            // slot
            ...
            let current_block =
                Self::align_offset(start, Self::allocation_size(start));
            let next_block = Self::align_offset(
                next.start(),
                Self::allocation_size(next.start()),
            );
            ensure!(current_block == next_block, continue);
            ensure!(slot.can_unsplit(next), continue);
    ...
    ///
    /// This can be used for copy-avoidance applications where a packet is
    /// received in order and doesn't need to be stored temporarily for
    /// future packets to unblock the stream.
    #[inline]
    pub fn skip(&mut self, len: VarInt) -> Result<(), Error> {
        ...
        // record the maximum offset that we've seen
        self.cursors.max_recv_offset =
            self.cursors.max_recv_offset.max(new_start_offset.as_u64());
    ...
    /// Pops the buffered length from the front of the receive queue without
    /// materializing payload.
    #[inline]
    pub fn pop_len(&mut self) -> Option<usize> {
        self.pop_len_watermarked(usize::MAX)
    }

    /// Pops the buffered length from the front of the receive queue, whose
    /// length is guaranteed to be less than `watermark`.
    #[inline]
    pub fn pop_len_watermarked(&mut self, watermark: usize) -> Option<usize> {
        let len = {
            let slot = self.slots.front_mut()?;

            ensure!(slot.is_occupied(self.cursors.start_offset), None);

            if self.cursors.final_size().is_some_and(|final_size| {
                final_size <= slot.end_allocated() &&
                    watermark >= slot.buffered_len()
            }) {
                slot.consume_all_buffered()
            } else {
                slot.consume_len(watermark)
            }
        };

        ensure!(len > 0, None);

        if self.slots.front().is_some_and(|slot| slot.should_drop()) {
    ...
    /// Pops a buffer from the front of the receive queue, who's length is
    /// always guaranteed to be less than the provided `watermark`.
    #[inline]
    pub fn pop_watermarked(&mut self, watermark: usize) -> Option<BytesMut> {
        let chunk = self.infallible_read_chunk(watermark);
    ...
    ///
    /// This includes the already consumed data as well as the data that is
    /// still buffered and available for consumption.
    #[inline]
        ...
        R: Reader + ?Sized,
    {
        ...
        {
            let size_candidate =
                (start - offset) as usize + reader.buffered_len();
        ...
        if reader.is_virtually_buffered() {
            Slot::new_virtual(offset, end)
        } else {
    ...
    ///
    /// The allocation size gradually increases as the offset increases. This is
    /// under the assumption that streams that receive a lot of data will
    /// continue to receive a lot of data.
    ///
    ...
    fn invariants(&self) {
        if cfg!(debug_assertions) {
            ...
            for (idx, slot) in self.slots.iter().enumerate() {
                ...
                // make sure if the slot is full, then it was unsplit into the
                // next slot
                if slot.is_full() {
                    let start = slot.start();
                    let end = slot.end();
